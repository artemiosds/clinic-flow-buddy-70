    ...
    FOR r_falta IN
        WITH all_absences AS (
            ...
            SELECT 
                ...
                '00:00' as hora_falta -- treatment_sessions não tem hora explícita as vezes, usamos 00:00 para ordenação
            FROM public.treatment_sessions
            WHERE patient_id = p_paciente_id
              AND status = 'falta'
              -- treatment_sessions pode ter campos similares no futuro, por ora pegamos status 'falta'
        )
        SELECT 
            profissional_id, 
            count(*) as total, 
