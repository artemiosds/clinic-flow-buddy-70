      ...
      >
        <DialogContent className="w-[95vw] max-w-5xl h-[95vh] max-h-[95vh] flex flex-col overflow-hidden" onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()}>
          ...
          <div className="space-y-4">
            ...
            {/* ═══ CAMPOS DINÂMICOS — sincronizados com Configurações > Prontuário ═══ */}
            {(() => {
              const camposDinamicos = getCamposForTipo(form.tipo_registro);
              const hasDynamic = camposDinamicos.length > 0;
              const TIPO_COLORS: Record<string, string> = {
                avaliacao_inicial: 'bg-green-500/5 border-green-500/20',
                retorno: 'bg-blue-500/5 border-blue-500/20',
                sessao: 'bg-amber-500/5 border-amber-500/20',
                urgencia: 'bg-destructive/5 border-destructive/20',
              ...
              return (
                <div className="space-y-4">
                  {/* Triagem data for urgência */}
                  {form.tipo_registro === 'urgencia' && triagem && (
                  ...
                  {/* Retorno: resumo do último atendimento */}
                  {form.tipo_registro === 'retorno' && patientHistory.length > 0 && (
                    ...
                    <>
                      {sessaoDataLoading ? (
                        ...
                        <>
                            ...
                            {sessaoCycle ? (
                              <>
                                ...
                                <div className="space-y-1">
                                  <div className="flex justify-between text-xs text-muted-foreground"><span>Sessão {sessaoCycle.sessions_done} de {sessaoCycle.total_sessions} realizadas</span><span>{Math.round((sessaoCycle.sessions_done / sessaoCycle.total_sessions) * 100)}%</span></div>
                                ...
                                {sessaoCycleSessions.length > 0 && (
                                  <div className="border rounded-md overflow-hidden">
                                    <table className="w-full text-xs">
                                      <thead className="bg-muted/50"><tr><th className="px-2 py-1.5 text-left font-medium text-muted-foreground">Nº</th><th className="px-2 py-1.5 text-left font-medium text-muted-foreground">Data</th><th className="px-2 py-1.5 text-left font-medium text-muted-foreground">Status</th><th className="px-2 py-1.5 text-right font-medium text-muted-foreground">Ação</th></tr></thead>
                                      <tbody>
                                        {sessaoCycleSessions.map(s => {
                                          ...
                                          return (
                                            <tr key={s.id} className={`border-t ${isCurrent ? 'bg-primary/5' : ''}`}>
                                              ...
                                              <td className="px-2 py-1.5 text-right">{isPending && !isRealizada && (<Button size="sm" variant={isCurrent ? "default" : "outline"} className="h-6 text-xs px-2" onClick={() => handleConfirmSession(s)} disabled={isConfirming || saving}>{isConfirming ? 'Confirmando...' : '✓ Confirmar'}</Button>)}</td>
                                            </tr>
                                          );
                                        })}
                                      </tbody>
                          ...
                          <div className="rounded-lg border bg-card p-4 space-y-3">
                            <h4 className="text-sm font-semibold text-foreground flex items-center gap-2"><ClipboardList className="w-4 h-4 text-primary" /> PTS Vinculado</h4>
                            {sessaoPts ? (() => {
                              ...
                              return (<>
                                {isOutdated && <Badge variant="outline" className="bg-warning/10 text-warning border-warning/30"><AlertTriangle className="w-3 h-3 mr-1" /> PTS desatualizado — revisar</Badge>}
                                <div className="text-sm space-y-2">
                                  ...
                                  <div className={`rounded-md p-2 border ${metaPeriod === 'curto' ? 'bg-primary/5 border-primary/20' : metaPeriod === 'medio' ? 'bg-accent border-accent-foreground/10' : 'bg-muted/50 border-border'}`}>
                                    <span className="text-xs font-semibold text-muted-foreground">{metaPeriod === 'curto' ? '🎯 Metas Curto Prazo (1-3 meses)' : metaPeriod === 'medio' ? '📋 Metas Médio Prazo (3-6 meses)' : '🔭 Metas Longo Prazo (6+ meses)'}</span>
                                    <p className="text-foreground text-sm mt-1">{metaPeriod === 'curto' ? sessaoPts.metas_curto_prazo : metaPeriod === 'medio' ? sessaoPts.metas_medio_prazo : sessaoPts.metas_longo_prazo}</p>
                                  </div>
                                  <div className="flex flex-wrap gap-1">{sessaoPts.especialidades_envolvidas.map(e => <Badge key={e} variant="secondary" className="text-xs">{e}</Badge>)}</div>
                                  <div className="text-xs text-muted-foreground">Criado em {new Date(sessaoPts.created_at).toLocaleDateString('pt-BR')}{profName && ` por ${profName}`}</div>
                                </div>
                              </>);
                            })() : (
                              <div className="space-y-2">
                          ...
                          {currentSessionForRegistration && sessaoHighlightSOAP && (
                            <div className="bg-primary/10 border border-primary/30 rounded-md p-2 text-sm text-primary font-medium">
                  ...
                  <div className={`rounded-lg border p-3 ${colorClass}`}>
                    ...
                    {hasDynamic ? (
                      <DynamicProntuarioFields
                        ...
                        onFormChange={(key, value) => setForm(p => ({ ...p, [key]: value }))}
                        onCustomChange={(key, value) => setCustomFields(p => ({ ...p, [key]: value }))}
                      />
                    ) : (
                  ...
                  {/* Decisão Clínica */}
                  {form.tipo_registro === 'avaliacao_inicial' && !editId && form.paciente_id && (
                    <div className="bg-muted/30 rounded-lg p-4 border space-y-3">
                      <h3 className="text-sm font-semibold text-foreground flex items-center gap-2"><Heart className="w-4 h-4 text-primary" /> Decisão Clínica (opcional)</h3>
                      <p className="text-xs text-muted-foreground">Crie PTS ou ciclo de tratamento para este paciente.</p>
                      <div className="flex gap-2 flex-wrap">
                        <Button type="button" variant="outline" size="sm" onClick={() => setPtsOpen(true)}><ClipboardList className="w-3.5 h-3.5 mr-1" /> Criar PTS</Button>
                        <Button type="button" variant="outline" size="sm" onClick={() => setCycleOpen(true)}><Activity className="w-3.5 h-3.5 mr-1" /> Criar Ciclo de Tratamento</Button>
                      </div>
                    </div>
                  )}
                </div>
